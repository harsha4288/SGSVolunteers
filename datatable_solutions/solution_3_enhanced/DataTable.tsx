...
const DataTable = React.forwardRef<HTMLDivElement, DataTableProps>(
  ({ 
    ...
    // Dynamic frozen column positioning (copied from Solution 7.1)
    React.useEffect(() => {
      if (frozenColumns.length === 0 || !tableRef.current) return;
      ...
      const columnsPerRow = firstRow ? firstRow.children.length : 0;
      ...
      frozenColumns.forEach((columnIndex) => {
    ...
    }, [frozenColumns, measurements]); // Re-run when measurements change
      ...
      <DataTableContext.Provider value={contextValue}>
        ...
        >
          <div
            ...
            style={{ maxHeight, paddingRight: "16px", overflowX: "auto", overflowY: "auto" }}
          >
            <table 
              ref={tableRef}
              className={cn(
                ...
                "min-w-full",
                frozenColumns.length > 0 && frozenColumns.includes(0) && "with-frozen-first"
              )}
...
const DataTableHead = React.forwardRef<HTMLTableCellElement, DataTableHeadProps>(
  ({ children, className, align = "left", vAlign = "middle", border = true, sticky = false, rowSpan, colSpan, colIndex, ...props }, ref) => {
    const { density, useDynamicSizing } = React.useContext(DataTableContext);
    
    // Frozen column styling is now handled by the central useEffect in DataTable component

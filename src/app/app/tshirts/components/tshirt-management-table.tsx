...
interface TShirtManagementTableProps {
...
export function TShirtManagementTable({
  supabase,
  isAdmin,
  eventId,
  tshirtSizes,
  ...
  React.useEffect(() => {
    ...
    const fetchVolunteerData = async () => {
      setLoading(true);
      try {
        console.log("Fetching data for volunteers:", volunteersToDisplay);
        ...
        volunteersToDisplay.forEach(vol => {
          ...
          console.log(`Volunteer ${vol.first_name} ${vol.last_name} (${vol.id}) allocation: ${newAllocations[vol.id]}`);
        });
        ...
        try {
          ...
          console.log(`Fetching issuances for volunteers: ${volunteerIds.join(', ')}`);
          ...
          if (issuanceError) {
            ...
            console.log(`Found ${issuanceData.length} issuances`);
        ...
        try {
          // Fetch preferences
          console.log(`Fetching preferences for volunteers: ${volunteerIds.join(', ')}`);
          ...
          if (prefsError) {
            ...
            console.log(`Found ${prefsData.length} preferences`);
            console.log("Preference data:", prefsData);
  ...
  const togglePreference = async (volunteerId: string, sizeId: string) => {
    ...
    try {
      ...
      if (!fetchError && data) {
        ...
        revertedPreferences[volunteerId] = {};

        data.forEach(pref => {
          revertedPreferences[volunteerId][pref.tshirt_size_id] = true;
        });
                ...
                <TableCell className="border-b">
                  {allocations[vol.id] > 0 ? (
                    ...
                    <span className="text-muted-foreground text-sm">
                      {issuances[vol.id]?.length ? "All allocated" : "No allocation"}
                    </span>
                  )}
                </TableCell>
              )}

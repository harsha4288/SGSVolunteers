...
export const createClient = () => {
  ...
  if (!supabaseUrl || supabaseUrl === "YOUR_SUPABASE_URL_HERE" || supabaseUrl === "" || typeof supabaseUrl === 'undefined') {
  ...
  if (!supabaseAnonKey || supabaseAnonKey === "YOUR_SUPABASE_ANON_KEY_HERE" || supabaseAnonKey === "" || typeof supabaseAnonKey === 'undefined') {
  ...
  try {
    // Ensure it starts with http:// or https:// before passing to new URL()
    if (!supabaseUrl.startsWith('http://') && !supabaseUrl.startsWith('https://')) {
        throw new Error(`Invalid URL format. URL must start with http:// or https://. Received: ${supabaseUrl}`);
    }
    new URL(supabaseUrl); // This validates the URL format
  } catch (e: any) {

...
export function RequirementsTable({ requirementsData }: RequirementsTableProps) {
  ...
  // Filter and flatten gridData to get the rows for this table
  ...
  return (
    <div className="mt-4">
      ...
      >
        ...
        <DataTableHeader>
          <DataTableRow>
            <DataTableHead vAlign="middle" colIndex={0} className="px-3">Location</DataTableHead>
        ...
        <DataTableBody>
          {requirementRows.map((row) => (
            <DataTableRow key={`${row.location_id}-${row.timeslot_id}`}>
              <DataTableCell vAlign="middle" overflowHandling="tooltip" tooltipContent={row.location_name} colIndex={0} className="px-3">
                {row.location_name}
              </DataTableCell>
                ...
                <InlineCountEditor
                  ...
                  onSave={(newCount) =>
                    updateRequirementsForCell(
                      ...
                      row.timeslot_id,
                      [{ location_id: row.location_id, required_count: newCount }]
                    )
                  }
